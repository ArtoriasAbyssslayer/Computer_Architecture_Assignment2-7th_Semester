...
**Best Deal**
* specbzip
Οσον αφορά αυτό το benchmark μπορούμε να δουμε το case C οπου εχουμε 1MB L2_size και μέσο cp1 1.55 (+0.01 από το best case D1) και χαμηλότερο associativity έχει κόστος 
  * cost = 368 
Πολύ καλό trade-off (cost-120.8)
* spechmmer
Οσον αφορά αυτό το benchmark μπορούμε να πάρουμε το case A 64KB = l1_size = l2_size  512KB = l2_size και χαμηλότερο associativity με μέσο cpi = 1.179(+0.001 από το besτ case C1) και έχει κόστος
   * cost = 172
Trade-off (cost-152)
* specsjeng 
Όσον αφορά το συγκεκριμένο benchmark , αυτό είναι αρκετά απαιτητικό σε πόρους(για το best case D1). Μπορούμε βέβαια να πάρουμε το case A με μέσο cpi = 3.715(+0.001 απο cpi D1) που έχει κόστος:
   * cost = 192
Trade-off (costD1 -316.8)
* specmcf
Μπορούμε να πάρουμε το B(cpiB1 + 0.0013) με κόστος:
  * cost = 283.8
Trade-off(costB1 -158 , cpiB1 +0.0013)
* speclibm
Μπορούμε να πάρουμε το Α(cpiB1 +0.0013) με κόστος:
   * cost = 192
Trade-off (costB1 -284.8 cpiB1 +0.0013)</br>
(Τα Α,Β,C,D,A1,B1,C1,D1 είναι στο 2)
#### Παρατηρήσεις
* Βλέποντας τα best deals για κάθε  περίπτωση παρατηρούμε οτι καθοριστικό ρόλο στην απόδωση παίζει κυρίως το cache line size  και όχι τα υπόλοιπα στοιχεία (το οποίο το είχα παρατηρήσει εξ'αρχής). Έτσι επιλέγουμε σε κάθε αρχιτκτονική να χρησιμοποιήσουμε τα ελάχιστα δυνατά μεγέθη μνημών(που είναι και τα πιο ακριβά) θυσιάζοντας μια μικρή βελτίωση στο cpi της τάξης του 0.001(αναλυτικά πάνω).
* Η συνάρτηση κόστους μας, ίσως τιμωρεί παραπάνω μεγάλες σε μέγεθος μνήμες l1,l2 έχοντας υψηλό συντελεστή κόστους σε αυτά και ο συντελεστής κόστους στα υπόλοιπα είναι ανάλογος με την λειτουργία τους(χαμηλότερος/οχι συγκρίσιμος με το κόστος ολόκληρης της μνήμης).

 ### Kριτική εργασίας**</br>
Η εργασία μας έβαλε στην λογική του αρχιτέκτονα επεξεργαστών, μας έφερε αντιμέτωπους με ζητήματα που προκύπτουν σχετικά με την σχεδίαση , την απόδοση και το κόστος ενός επεξεργαστή.</br>
Είδαμε οτι δεν υπάρχει κάποια συγκεκριμένη μέθοδος στην επίλυση των παραπάνω ζητημάτων αλλά έπρεπε να εξασκήσουμε την φαντασία μας και να κάνουμε πολλές δοκιμές(χρονοβόρες).</br>
Στο εργαστήριο 2 γίνεται εφαρμογή όσων αναφέρονται στην θεωρία σε πιο πρακτικό επίπεδο.</br>
